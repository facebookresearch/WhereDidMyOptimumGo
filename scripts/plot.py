        ...
        for s, seed_dir in enumerate(get_dirs(game_dir)):
            ...
            for dir in get_dirs(seed_dir, dir_key):
                ...
                if "LBFGS" in algo_name:
                    continue # LBFGS currently not working so well
                ...
                game_replay_key += "(lr: {})".format((re.findall('\d+\.\d+', local_dir[local_dir.rfind('lr'):min(local_dir.rfind('lr')+10, len(local_dir))] ))[0])
                ...
                if tx is not None and ty is not None:
                    add_to_plot_dict('Reward', monitor_dict, game_replay_key, tx, ty, int(1e7), algo_name)
