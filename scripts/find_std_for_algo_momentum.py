...
def log_avg_to_csv(monitor_dict, csv_loc, num_episodes=100):
    ...
    for game_replay_key in sorted(monitor_dict):
        ...
        for monitor_type in sorted(monitor_dict[game_replay_key]):
            for lr in sorted(monitor_dict[game_replay_key][monitor_type]):
                ...
                mean = [str(avg) for avg in np.mean(mean).reshape(-1)]
...
def get_all(viz, log_dir, dir_key='.*', save_loc=None):
    ...
    for game_dir in get_dirs(log_dir):
        ...
        for s, seed_dir in enumerate(get_dirs(game_dir)):
            ...
            for dir in get_dirs(seed_dir, dir_key):
                ...
                if 'LBFGS' in algo_name or "YFOptim" in algo_name:
                    continue
        ...
        log_avg_to_csv(monitor_dict, csv_loc=save_loc)
    return
...
if __name__ == "__main__":
    args = parse_args()
    viz = None
    get_all(viz, args.log_dir, save_loc=args.save_loc)
