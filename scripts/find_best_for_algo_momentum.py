...
def log_avg_to_csv(monitor_dict, csv_loc, real_num_episodes=None, momentum=False, std=False):
    keys = list(monitor_dict.keys())
    all_algos = [monitor_dict[keys[0]][monitor_type][lr]['algo'] for monitor_type in monitor_dict[keys[0]] for lr in monitor_dict[keys[0]][monitor_type]][0]
    algos = []
    ...
    for game_replay_key in sorted(monitor_dict):
        if std:
            with open(csv_loc + game_replay_key + 'std_results.csv', "w") as csv_file:
                csv_writer = csv.writer(csv_file, delimiter ='&')
                if momentum:
                    csv_writer.writerow(['momentum'] + algos)
                else:
                    csv_writer.writerow(['lr'] + algos)
        else:
            with open(csv_loc + game_replay_key + 'avg_results.csv', "w") as csv_file:
                csv_writer = csv.writer(csv_file, delimiter ='&')
                if momentum:
                    csv_writer.writerow(['momentum'] + algos)
                else:
        ...
        for monitor_type in sorted(monitor_dict[game_replay_key]):
            for lr in sorted(monitor_dict[game_replay_key][monitor_type]):
                ys = monitor_dict[game_replay_key][monitor_type][lr]['y']
                # print(len(xs), len(ys), game_replay_key, monitor_type)
                if real_num_episodes is None:
                    ep_len=  min([len(z) for z in ys])
                    num_episodes = ep_len

                data = np.empty((len(ys), num_episodes))
                for i in range(len(ys)):
                    data[i] = ys[i][-num_episodes:]
                # data, _ = align_data(xs, ys)

                print(data.shape)
                #
                data = reshaped_data(data, monitor_dict[game_replay_key][monitor_type][lr]['algo'], algos)
                if std:
                    mean = np.std(data, axis=(-1))
                    mean = [str(int(avg)) for avg in np.mean(mean, -1)]
                    with open(csv_loc + game_replay_key + 'std_results.csv', "a") as csv_file:
                        csv_writer = csv.writer(csv_file, delimiter ='&')
                        csv_writer.writerow([lr] + mean)
                else:
                    mean = np.mean(data, axis=(-2, -1))
                    mean = [str(int(avg)) for avg in mean]
                    with open(csv_loc + game_replay_key + 'avg_results.csv', "a") as csv_file:
                        csv_writer = csv.writer(csv_file, delimiter ='&')
                        csv_writer.writerow([lr] + mean)
...
def visdom_plot_batched(viz, win, x, y, title, name, bin_size=1, smooth=1, x_label='Number of Epochs',
    ...
    if type(name) is list:
        ...
        for method in methods:
            ...
            plt.plot(xs, mean, label="{}".format(method), color=color, linewidth=2)
...
def add_to_plot_dict(monitor_type, monitor_dict, game_replay_key, x, y, max_x, algo_name, lr):
    if monitor_type in monitor_dict[game_replay_key] and lr in monitor_dict[game_replay_key][monitor_type]:
        monitor_dict[game_replay_key][monitor_type][lr]['x'].append(x)
        monitor_dict[game_replay_key][monitor_type][lr]['y'].append(y)
        monitor_dict[game_replay_key][monitor_type][lr]['max_x'].append(max_x)
        monitor_dict[game_replay_key][monitor_type][lr]['algo'].append(algo_name)
    elif monitor_type not in monitor_dict[game_replay_key] :
        monitor_dict[game_replay_key][monitor_type] = {}
        monitor_dict[game_replay_key][monitor_type][lr] = {'x': [x], 'y': [y], 'max_x': [max_x],
                                                       'algo': [algo_name]}
    elif lr not in monitor_dict[game_replay_key][monitor_type] :
        monitor_dict[game_replay_key][monitor_type][lr] = {'x': [x], 'y': [y], 'max_x': [max_x],
                                                       'algo': [algo_name]}
...
def plot_dict(monitor_dict, viz, save_loc=None):
    ...
    for game_replay_key in monitor_dict:
        for monitor_type in monitor_dict[game_replay_key]:
            # only plot the best lr based on the averaged last few ys
...
def get_all(viz, log_dir, dir_key='.*', save_loc=None, momentum=False, num_episodes=None, std=False):
    ...
    for game_dir in get_dirs(log_dir):
        ...
        for s, seed_dir in enumerate(get_dirs(game_dir)):
            ...
            for dir in get_dirs(seed_dir, dir_key):
                ...
                tx, ty = load_data(dir, smooth=0, bin_size=100)

                if tx is not None and ty is not None:
                    add_to_plot_dict('Reward', monitor_dict, game_replay_key, tx, ty, int(1e7), algo_name, lr)
        log_avg_to_csv(monitor_dict, csv_loc=save_loc, momentum=momentum, real_num_episodes=num_episodes, std=std)
    return
...
def parse_args():
    ...
    parser.add_argument("--num_episodes", default=None, type=int, help="Number of episodes to average over, if None, uses the average over all")
    parser.add_argument("--std", action="store_true", help="calculate the std instead of the mean")
...
if __name__ == "__main__":
    args = parse_args()
    viz = None
    get_all(viz, args.log_dir, save_loc=args.save_loc, momentum=args.momentum, num_episodes=args.num_episodes, std=args.std)
